...
import { Card, Text , Title,DonutChart,BarChart ,BarList,Metric ,Grid,Col,LineChart,SparkAreaChart, SparkLineChart, SparkBarChart  } from "@tremor/react";
...
export default function StatisticsBasic({jobs}) { 
  ...
  const statusCounts = Object.entries(jobs.reduce((counts,job)=>{
...
const jobsAppliedMoreThan45DaysAgo = jobs.filter(job => {
  const jobDate = new Date(job.dateApplied);
  jobDate.setHours(0, 0, 0, 0); // Set the time to 00:00:00
  return jobDate.getTime() < fortyFiveDaysAgo.getTime() && job.status === 'Applied';
}).length;

const appliedObject = statusCounts.find(item => item.name === 'Applied');
const appliedStatusCount = appliedObject ? appliedObject.value : 0;
const possibleApplications: number = appliedStatusCount as number - jobsAppliedMoreThan45DaysAgo;
...
const numJobsAppliedToday = jobsAppliedToday.length;
    ...
    return (
      ...
      <Col >
          ...
          <BarList showAnimation={true} data={statusCounts} className="mt-2" />
          {/* <DonutChart
            data={statusCounts}
            category="value"
            index="name"
